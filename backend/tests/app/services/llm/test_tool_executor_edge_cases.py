...
class TestToolExecutorEdgeCases:
    ...
    async def test_execute_with_none_result(self):
        ...
        with patch.object(
            executor.registry, "get_tool_function", return_value=mock_tool_func
        ):
            tool_call = ToolCall(
                id="test_none",
                type="function",
                function=ToolCallFunction(name="none_tool", arguments="{}"),
            )
    ...
    async def test_execute_with_sync_function(self):
        ...
        with patch.object(
            executor.registry, "get_tool_function", return_value=async_wrapped_sync_func
        ):
            tool_call = ToolCall(
                id="test_sync",
                type="function",
                function=ToolCallFunction(name="sync_tool", arguments="{}"),
            )
    ...
    async def test_execute_with_concurrent_calls(self):
        ...
        with patch.object(
            executor.registry, "get_tool_function", side_effect=get_tool_side_effect
        ):
            tool_calls = [
                ToolCall(
                    id=f"call_{i}",
    ...
    async def test_serialization_with_complex_types(self):
        ...
        with patch.object(
            ...
            tool_call = ToolCall(
                id="test_custom",
                type="function",
                function=ToolCallFunction(name="custom_tool", arguments="{}"),
            )
        ...
        with patch.object(
            ...
            with patch("app.services.llm.tool_executor.logger") as mock_logger:
                tool_call = ToolCall(
                    id="test_log",
                    type="function",
                    function=ToolCallFunction(name="logged_tool", arguments="{}"),
                )
                ...
                assert any("exec_123" in str(call) for call in log_calls)
