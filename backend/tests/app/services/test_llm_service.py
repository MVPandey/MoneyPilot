...
class TestLLMService:
    ...
    async def test_query_llm_simple_text(self, mock_client_class):
        ...
        message = await service.query_llm(
    ...
    async def test_query_llm_json_response(self, mock_client_class):
        ...
        result = await service.query_llm(
            Message(role="user", content="Get status"),
            json_response=True
        )
    ...
    def test_prepare_tools(self):
        ...
        with patch.object(service.tool_executor.registry, 'get_tool_schemas') as mock_get_schemas:
    ...
    async def test_query_llm_with_tools(self, mock_client_class):
        ...
        with patch.object(LLMService, '_prepare_tools') as mock_prepare:
            mock_prepare.return_value = [{"type": "function", "function": {"name": "test_tool"}}]
            ...
            service.tool_executor.execute_tool_calls = AsyncMock(return_value=[tool_result])
    ...
    async def test_query_llm_handles_json_decode_fallback(self, mock_client_class):
        ...
        mock_completion = self.create_mock_completion('```json\n{"result": "cleaned"}\n```')
        mock_async_client.chat.completions.create = AsyncMock(return_value=mock_completion)
    ...
    def test_process_response_non_json(self):
        ...
        result = service._process_response(completion, json_response=False, request_id="test")
    ...
    def test_process_response_json_success(self):
        ...
        result = service._process_response(completion, json_response=True, request_id="test")
